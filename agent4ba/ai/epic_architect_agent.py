"""Epic Architect agent for generating high-level features from business objectives."""
...
def load_generate_epics_prompt() -> dict[str, Any]:
    """
    Charge le prompt de génération d'epics depuis le fichier YAML.

    Returns:
        Dictionnaire contenant le prompt et les exemples
    """
    prompt_path = Path(__file__).parent.parent.parent / "prompts" / "generate_epics.yaml"
...
def generate_epics(state: Any) -> dict[str, Any]:
    """
    Génère une liste exhaustive de features de haut niveau à partir d'un objectif métier.

    Cet agent a pour unique responsabilité de produire des FEATURES/EPICS,
    JAMAIS de user stories ou de tâches techniques.
    ...
    """
    logger.info("Generating high-level features (epics) from business objective...")
    ...
    if not objective:
        ...
        return {
            "status": "error",
            "result": "No objective provided for epic generation",
        }
    ...
    start_event = {
        "type": "agent_start",
        "thought": f"Parfait ! Je vais identifier l'ensemble des features de haut niveau pour « {objective} ».",
        "agent_name": "EpicArchitectAgent",
    }
    ...
    plan_event = {
        ...
        "steps": [
            ...
            "Identification des grands chantiers fonctionnels (Features)",
    ...
    # Charger le prompt
    prompt_config = load_generate_epics_prompt()
    ...
    llm_event = {
        ...
        "tool_name": "Appel LLM",
        "tool_icon": "🧠",
        "description": f"Génération des features avec {model}",
        "status": "running",
    ...
    try:
        ...
        logger.info(f"Generated {len(work_items_data)} features")

        # Vérifier que tous les items sont bien des features
        non_feature_items = [item for item in work_items_data if item.get("type") != "feature"]
        if non_feature_items:
            logger.warning(f"Found {len(non_feature_items)} non-feature items in response")
        ...
        logger.info(f"- {len(new_items)} new features")
        ...
        plan_build_event = {
            ...
            "tool_icon": "📋",
            "description": "Création du plan d'impact avec les features générées",
            "status": "completed",
        ...
        return {
            ...
            "result": f"Generated {len(new_items)} high-level features for objective: {objective}",
        ...
        logger.error("Error during epic generation.", exc_info=True)
        ...
        return {
            "status": "error",
            "result": f"Failed to generate epics: {e}",
            "agent_events": agent_events,
        }
